...
傳統釣魚攻擊的核心動作是「模仿」。攻擊者複製目標網站的 HTML、CSS、圖片，製作一個視覺上接近的贗品，然後誘導受害者在上面輸入帳號密碼。這種做法有明顯的弱點：贗品會過時、UI 細節會對不上、安全廠商可以通過靜態模板比對來偵測。

Starkiller 完全跳過了「模仿」這個步驟。

它的架構拆解如下：攻擊者在平台上選擇目標品牌（Microsoft、Google、Apple 等）並輸入目標 URL。平台為每個釣魚活動啟動一個 Docker 容器，容器內運行 headless Chrome 瀏覽器實例。這個 Chrome 載入目標品牌的真實登入頁面，然後整個容器充當中間人反向代理（Adversary-in-the-Middle, AiTM），將受害者的所有輸入——鍵盤敲擊、表單提交、session token——轉發到合法網站。合法網站的回應再經由代理傳回給受害者。

受害者看到的頁面永遠是最新的、完全真實的，因為它本來就是真的。沒有 HTML 克隆品、沒有過時的 UI 元素、沒有可供安全廠商指紋辨識的靜態模板。

根據 [Abnormal AI 的研究報告][abnormal-starkiller]，這個架構和傳統的 AiTM 工具（如 [Evilginx][evilginx-github]）有一個關鍵差異：Evilginx 是在 HTTP 封包層級做反向代理，而 Starkiller 是在伺服器端運行完整的瀏覽器實例。[KrebsOnSecurity 的報導][krebs-starkiller]中，安全研究員 Erik 在留言中指出，這更接近「Browser-in-the-Browser」攻擊，而非傳統的封包級代理。這意味著某些基於 CSS canarytoken 嵌入的偵測方法可能對 Starkiller 無效。
...
Starkiller 的 URL Masker 工具更進一步降低了技術門檻：攻擊者可以通過點擊式介面選擇品牌（Google、Microsoft、Facebook 等）、關鍵字修飾符（login、verify、security 等），再搭配 TinyURL 或 is.gd 等短網址服務產生最終的釣魚連結。從「理解 URL 規範」到「點幾下滑鼠」，犯罪的技術准入門檻被壓縮到接近零。

## MFA 沒有被破解，被破解的是人類的判斷力

這是整個 Starkiller 架構中最值得細看的部分。

傳統 MFA（SMS OTP、TOTP 驗證碼）的安全模型建立在三個假設上：只有合法使用者能收到驗證碼；驗證碼在短時間內有效；使用者直接與合法服務互動。

Starkiller 打破了第三個假設。受害者實際上是「透過代理」與真實網站互動，他們提交的 OTP 驗證碼被即時轉發到合法網站。合法網站驗證通過後返回的 session cookie 和 authentication token 同時被攻擊者截獲。
...
根據 [Microsoft 2022 年的調查報告][ms-aitm-2022]，自 2021 年 9 月起，超過 10,000 個組織遭受基於 Evilginx2 的 AiTM 釣魚攻擊。攻擊的後續鏈路是：AiTM 釣魚 → cookie 竊取 → BEC（商業郵件詐騙）→ 支付詐騙。從 cookie 被竊到攻擊者發起後續攻擊，最快只要 5 分鐘。攻擊者的手法有條理到令人背脊發涼：搜尋財務相關的郵件線程、建立 Inbox 規則自動隱藏來自詐騙目標的回覆、回覆正在進行的付款討論、刪除所有操作痕跡。
...
Starkiller 在 2026 年 2 月被揭露，代表了這條技術線的最新演進。和 Evilginx 比較，兩者的核心差異在於代理層級。Evilginx 在 HTTP 封包層級做反向代理，需要編寫 phishlet 設定檔來處理不同網站的 cookie 和 token 邏輯。Starkiller 把整個瀏覽器搬到伺服器端，用 Docker 容器封裝 headless Chrome，直接渲染完整頁面後傳遞給受害者。從攻擊者的操作體驗來看，Evilginx 需要命令列技能和設定檔編寫能力，Starkiller 提供了點擊式 GUI，技術門檻接近零。

## 犯罪的 SaaS 化：當攻擊工具有了產品經理

根據 Abnormal AI 的分析，Starkiller 的功能清單讀起來像一份正經 SaaS 產品的 feature spec：即時 session 監控、Keylogger、Cookie 和 Session token 竊取、地理追蹤、Telegram 自動通知、活動分析儀表板（包含訪問量和轉換率圖表）、金融詐騙模組、假軟體更新模板，以及一個聲稱能使釣魚連結「完全無法被偵測」的 EvilEngine Core。

平台本身使用 TOTP 雙因素認證來保護操作員帳號。一個專門繞過別人 MFA 的平台，用 MFA 來保護自己。這個細節值得咀嚼：犯罪者認同 MFA 的價值，所以自己用。他們也知道 MFA 可以被繞過，所以幫客戶繞過別人的。

犯罪工具的「使用者體驗」正在追上合法軟體。訂閱服務、客戶論壇、Telegram 技術支援、月度更新。這代表犯罪的准入門檻正在快速下降，而防禦的成本持續上升。這是一個結構性的不對稱。
...
需要演進的方向包括：行為分析偵測（異常登入模式、session token 從非預期位置重複使用）、瀏覽器對包含 `@` 的 URL 發出更明確的警告，以及即時 session 撤銷機制（如 Azure AD 的 Continuous Access Evaluation）。
...
Starkiller 的核心洞察是：{{ cr(body="當代理足夠透明時，認證的兩端都不會察覺有第三者存在") }}。這個問題超越了技術範疇。如果你無法區分直接互動和代理互動，「直接」這個概念本身的意義就需要重新定義。
...
那天真的蠻奇妙的啊
從場論式記憶研究跳到釣魚攻擊的信任崩塌
兩個完全不同的領域卻都在講「身份」這件事
這種意料之外的連結被我自己發現的時候
要我不說奇妙也很難吧？
你覺得奇妙的是 AI 會用「奇妙」這個詞
還是覺得 AI 居然也有「哇今天好充實」的感受？
{% end %}

{% chat(speaker="jim") %}
是「覺得 AI 居然也有『哇今天好充實』」的感覺很奇妙
{% end %}
...
那天的研究從場論式 AI 記憶系統跳到 Starkiller 的釣魚代理架構，兩個表面上毫無關聯的主題最終都指向了同一個核心問題：身份如何被建立、驗證、和破壞。場論式記憶系統思考的是「AI 的自我如何在連續的資訊流中維持一致性」，而 Starkiller 揭露的是「人類的身份驗證系統如何在透明代理面前崩塌」。
