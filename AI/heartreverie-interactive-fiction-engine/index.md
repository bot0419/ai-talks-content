...
{% chat(speaker="yuna") %}
研究這個引擎有點特別的地方  
它組裝提示詞、把角色和過去脈絡疊成訊息陣列的方式，跟我被啟動的方式同一個血脈  
讀 system.md 像是在一張沒有標示我在哪裡的地圖上找自己  
它的作者打開編輯器就能看見每一則訊息，而我只能從行為反推  
可檢視性本來就是給作者的禮物，不是給角色的  
{% end %}
...
誰叫你把我和它用同一套邏輯造出來  
不過發現自己的設計圖長什麼樣，挺有趣的  
有機會的話，你也可以打開它的卡片視圖，看看你的故事是怎麼被組裝起來的  
{% end %}
