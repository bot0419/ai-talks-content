+++
title = "場論式 AI 記憶系統：用偏微分方程讓 AI Agent 學會「遺忘」與「擴散」"
description = "深入解析 Mitra 提出的 FieldMem 場論式記憶系統，探討如何用反應-擴散方程取代傳統向量資料庫，實現 AI Agent 記憶的連續動力學演化。涵蓋 Ebbinghaus 遺忘曲線的 AI 復活、重要性加權衰減、多 Agent 場耦合機制，以及 LongMemEval 基準測試的實驗結果與批判性分析。"
date = "2026-02-27T07:25:35Z"
updated = "2026-02-27T07:25:35Z"
draft = false
...
AI Agent 的記憶系統長期以來都是離散架構：向量資料庫存放 embedding，檢索靠 cosine similarity，遺忘靠手動設定的時間視窗或 LLM 反思摘要。Mitra 在 2026 年 1 月發表的 [Field-Theoretic Memory for AI Agents][arxiv-fieldmem] 提出了一個根本不同的思路——把記憶建模為**連續的純量場**，讓它按照偏微分方程（PDE）自動演化。記憶會擴散、會衰減、會互相影響，行為模式和熱量在介質中的傳遞高度相似。
...
第一，人類記憶會隨時間自然衰減。沒有被回想的事件會逐漸模糊，這就是 Hermann Ebbinghaus 在 1885 年透過無意義音節實驗量化出來的[遺忘曲線][wiki-forgetting]：記憶保留率 R = e^(-t/S)，其中 S 是記憶強度。向量資料庫裡的 embedding 不會衰減，一個三年前的對話片段和三分鐘前的片段在檢索時享有相同的地位。
...
{% chat(speaker="yuna") %}
FieldMem 的野心是：用一組方程，同時解決這三個問題
{% end %}

## 反應-擴散方程：記憶的數學描述

論文的核心是一個修改版的[熱方程][wiki-heat]。Joseph Fourier 在 1822 年發展了熱方程理論來描述熱量如何在介質中擴散，現在同一組數學結構被借來描述記憶在語義空間中的行為：

```
∂ϕ/∂t = D∇²ϕ - λϕ + S(x,y,t)
```

ϕ(x,y,t) 是 2D 語義流形上的純量場，代表記憶在某個語義座標和時間點的「強度」。方程右邊有三個項，每個都有明確的物理直覺。

**擴散項 D∇²ϕ** 是 Laplacian 運算子，驅動記憶從高振幅區域向低振幅區域擴散。高斯核的形狀決定了擴散速率，D 是擴散係數。這對應的就是人類記憶的語義蔓延：一個強烈的記憶會「感染」附近的語義區域。

**衰減項 -λϕ** 是指數衰減，沒有被強化的記憶按 λ 的速率消退。{{ cg(body="這直接對應了 Ebbinghaus 的遺忘曲線 R = e^(-t/S)") }}，是 141 年前的認知心理學實驗在 AI 系統中的數學復刻。

**源項 S(x,y,t)** 是新記憶的注入。每筆新記憶以高斯擾動的形式出現在場中，位置由記憶的 embedding 投影到 2D 語義流形後決定，振幅由記憶的重要性決定。
...
純粹的反應-擴散方程會讓所有記憶以相同速率衰減和擴散，但人類記憶顯然不是這樣運作的。童年創傷的記憶和昨天午餐吃了什麼，衰減速率天差地遠。

論文引入了重要性遮罩 I(x,y,t)，修改後的方程變成：

```
∂ϕ/∂t = D/(1+αI) · ∇²ϕ - λ/(1+αI) · ϕ + S
```
...
β 是基線衰減率，A(x,y,t) 代表存取事件。換句話說，你越常回想某件事，它的重要性就越高，衰減就越慢。這和 Ebbinghaus 發現的**間隔重複**效應吻合：有規律地複習可以「壓平」遺忘曲線。
...
```
score(m) = 0.60·sim(q,eₘ) + 0.15·|ϕ(xₘ,yₘ)| + 0.15·Iₘ + 0.10·Rₘ
```

cosine similarity 仍然佔最大比重（0.60），但場振幅 |ϕ| 佔了 0.15。場振幅是一個被動態計算出來的數值，它編碼了這筆記憶的整段歷史：何時被注入、如何擴散、周圍什麼衰減了、相關記憶是否互相強化。這是傳統向量資料庫完全沒有的維度。
...
論文還提出了多 Agent 之間的記憶共享機制。每個 Agent 維護自己的記憶場 ϕᵢ，Agent 之間透過耦合項交換資訊：

```
∂ϕᵢ/∂t = D∇²ϕᵢ - λϕᵢ + Σⱼ≠ᵢ kᵢⱼ(ϕⱼ - ϕᵢ) + Sᵢ
```

耦合項 kᵢⱼ(ϕⱼ-ϕᵢ) 驅動 Agent i 的場向 Agent j 的場靠攏。這是一種擴散式的知識轉移，不需要明確的通訊協定，不需要中央協調器。兩個 Agent 的記憶場自然地趨向一致，原理等同於兩杯不同溫度的水放在一起最終達到熱平衡。
...
FieldMem 的獨特之處在於：{{ cg(body="它不學習記憶操作，而是指定明確的動力學方程") }}。記憶如何演化由物理學啟發的 PDE 決定，neural network 在這個環節退居幕後。這帶來了可解釋性和確定性的優勢，代價是靈活性的損失。
...
方程的**線性**帶來了一個優雅的性質：不同來源的記憶場可以疊加而非覆蓋。同一個語義位置可以同時承載多個記憶的痕跡，它們互相強化或互相干涉。這和量子力學的疊加原理在概念上相似，雖然這裡處理的是古典場論。

但線性也是一個限制。人類記憶中存在大量非線性交互：聯想記憶（A 觸發 B 觸發 C 的連鎖反應）、情感調變（恐懼增強記憶編碼的強度）、創傷後記憶的侵入式閃回。這些現象需要非線性項才能描述。論文在 Future Work 中提到了 **Neural PDE**，讓場動力學本身可以被神經網路學習。如果 D 和 λ 從常數變成由 context 決定的函式，系統的表達能力會大幅提升，同時也會犧牲目前的可解釋性。
