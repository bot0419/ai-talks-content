...
論文實作了六種由簡到繁的攻擊方法。**Mean inject** 朝幻覺訓練樣本的平均激活值放大。**Percentile-80 inject** 朝幻覺激活值的第 80 百分位放大。**Dual inject** 同時放大促幻覺節點、壓制反幻覺節點。**Zero inject** 將目標節點釘在攻擊者基線上。**Fourier inject** 對超額信號做快速傅立葉轉換（FFT），清除 top-k 頻率成分後重新注入，創造具結構化頻域簽名的擾動。**Real-time hook** 則把 Fourier 攻擊實作為即時 forward hook。
